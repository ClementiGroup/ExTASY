...
ref=Kconfig.md_reference 

if args.save_traj=='True':
  print('STARTING rg rmsd Q npy production')
  original_file = md.load(args.path+'/'+ref)
  selection = original_file.topology.select('name CA')
  original_file.atom_slice(selection, inplace=True)
  BETA_CONST = 50  # 1/nm
  LAMBDA_CONST = 1.8
  NATIVE_CUTOFF = 1.25  # nanometers
  #all atom #NATIVE_CUTOFF = 0.45  # nanometers
  ...
  for iter_now in range(args.iter+1):
      if not os.path.isfile(args.path+'/iter'+str(iter_now)+'_feat'+str(i)+'_rgrmsdq.npy') or iter_now==args.iter:
        print("iter:"+ str(iter_now)+ "npy "+str(i)+" from "+ str(args.idxend))
        for i in range(args.idxstart,args.idxend):
          time_start=time.time()
          file = args.path+'/iter'+str(iter_now)+'_out'+str(i)+'.pdb'
          if os.path.isfile(file):
            file2 = md.load(file)
            selection = file2.topology.select('name CA')
            file2.atom_slice(selection, inplace=True)
            rmsd_val=md.rmsd(file2,original_file)[0]
            r = md.compute_distances(file2[0], native_contacts)
            q = np.mean(1.0 / (1 + np.exp(BETA_CONST * (r - LAMBDA_CONST * r0))), axis=1)[0]
            vals=np.array([md.compute_rg(file2)[0], rmsd_val, q])
            np.save(args.path+'/iter'+str(iter_now)+'_feat'+str(i)+'_rgrmsdq.npy',vals)
            print('time rgrmsdq', str(i), "time", str(time.time()-time_start))
  top_file=glob.glob(args.path+'/iter*_out*.pdb')[0]
  print('STARTING rg rmsd Q 2 npy production')
  for iter_now in range(args.iter+1):
    if not os.path.isfile(args.path+'/iter'+str(iter_now)+'_feat'+str(i)+'_rgrmsdq2.npy') or iter_now==args.iter:
        print("iter:"+ str(iter_now)+ "npy "+str(i)+" from "+ str(args.idxend))
        for i in range(args.idxstart,args.idxend):
          time_start=time.time()
          file = args.path+'/iter'+str(iter_now)+'_traj'+str(i)+'.dcd'
          if os.path.isfile(file):
            file2 = md.load(file,top=top_file)
            selection = file2.topology.select('name CA')
            file2.atom_slice(selection, inplace=True)
            rg_val=md.compute_rg(file2)
            rmsd_val=md.rmsd(file2,original_file)#, atom_indices=heavy)
            r = md.compute_distances(file2, native_contacts)
            q = np.mean(1.0 / (1 + np.exp(BETA_CONST * (r - LAMBDA_CONST * r0))), axis=1)
            vals=np.stack([rg_val, rmsd_val, q])
            np.save(args.path+'/iter'+str(iter_now)+'_feat'+str(i)+'_rgrmsdq2.npy',vals)
            print('time rgrmsdq', str(i), "time", str(time.time()-time_start))
